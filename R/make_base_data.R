...
# remove because does not change the result of the analysis, except for
# a couple of more individuals in each group, but adds a black-box layer
# predictor_matrix <- read_csv("data/missing_predictor_matrix.csv") %>%
#   as.matrix() 
# row.names(predictor_matrix) <- colnames(predictor_matrix)
# missing <- PC18 %>% 
#   select(IDENT18, DIPLOME_pere, DIPLOME_mere, CS_pere,
#          CS_mere, AGE_5_r, SEXE_r, naiss_parents, DIPLOME_r, PCS_MENAGE,
